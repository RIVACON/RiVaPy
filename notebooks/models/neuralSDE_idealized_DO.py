...
class LatentSDE(nn.Module):
    ...
    def __init__(self, hidden_size,context_size,latent_size,input_size,sigma,kappa, theta): 
        ...
        self.encoder = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(hidden_size, latent_size),#context_size),
        )
        ...
        #self.qz0_net = #TODO nn.Linear(context_size, latent_size + latent_size)
        logvar = math.log(sigma ** 2 / (2. * kappa))
        self.qz0_mean = nn.Parameter(torch.tensor([[theta]]), requires_grad=True)
        self.qz0_logstd = nn.Parameter(torch.tensor([[logvar]]), requires_grad=True)


        self.latent_drift = nn.Sequential(
            nn.Linear(latent_size+1, hidden_size),#(latent_size + context_size, hidden_size),
            nn.ReLU(),
            nn.Linear(hidden_size, hidden_size),
            nn.ReLU(),
        ...
        #self.register_buffer("pz0_mean", torch.tensor([[theta]]))
        #self.register_buffer("pz0_logstd", torch.tensor([[logvar]]))
    ...
    def f(self, t, y): # (posterior) Drift 
        ...
        return self.latent_drift(torch.cat((t, y), dim=-1))
        #ts, ctx = self._ctx
        #i = min(torch.searchsorted(ts, t, right=True), len(ts) - 1)
        #return self.f_net(y)#torch.cat((y, ctx[i]), dim=1))
    ...
    def forward(self, xs, ts, noise_std):
        ...
        z0 = self.qz0_mean          #+ qz0_logstd.exp() * torch.randn_like(qz0_mean)

        zs, log_ratio = torchsde.sdeint(self, z0, ts,  dt=ts[1]-ts[0], logqp=True, method='euler', rtol = 1e-3, atol = 1e-3, names={'drift': 'f'})

        _xs = self.decoder(zs)
        xs_dist = Normal(loc=_xs, scale=noise_std)
        log_pxs = xs_dist.log_prob(xs).sum(dim=(0, 2)).mean(dim=0)

        qz0 = torch.distributions.Normal(loc=self.qz0_mean, scale=self.qz0_logstd.exp())
...
#----------------------- #testing decoder------------------------------------------------------------------------------------------
#testproj = torch.tensor([[inputmean[i]]for i in range(N)])
#testproj = latent_sde.projector(testproj).detach().numpy()
#axis[1].plot(tt,testproj, label = 'testproj',color='blue', linewidth = 4)
#axis[2].plot(tt,testproj, label = 'testproj',color='blue', linewidth = 4)
#---------------------------------------------------------------------------------------------------------------------
