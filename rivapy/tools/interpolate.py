...
class Interpolator:
    ...
    def __init__(self, interpolation_type: _Union[str, InterpolationType], extrapolation_type:_Union[str, ExtrapolationType]):
        ...
        self._extrapolation_type = ExtrapolationType.to_string(extrapolation_type) # TODO is this redundant as we feed extrapolation method into interp  as argument
    ...
    def interp(self,
               x_list: list,
               y_list: list,
               target_x: _Union[float, _List[float]],
               extrapolation: str
        ...
        if isinstance(target_x, list):
            return [self._interp(x_list, y_list, target_x_, extrapolation) for target_x_ in target_x]
        else:
            return self._interp(x_list, y_list, target_x, extrapolation)
    ...
    def linear(x_list: list, y_list: list, x: float, extrapolation:str) -> float:
        ...
        # TODO: remove once decision made 2025.07.03
        #y2 = interp1d(x_list, y_list, kind = 'linear', fill_value = 'extrapolate')
        #if y != 0 or y2 !=0 :
        #    if abs(y-y2) / max(abs(y), abs(y2)) > 0.0001 :
        #        print("Difference between scipy.interp1d and basic is more than 0.01%!")
