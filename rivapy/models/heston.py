...
class HestonModel:
    def __init__(self, long_run_variance, mean_reversion_speed, vol_of_vol, initial_variance, correlation):
        """_summary_
    ...
    def _characteristic_func(self, xi, s0, v0, tau):
        ...
        ixi = 1j * xi
        d = np.sqrt((self._mean_reversion_speed - ixi * self._correlation * self._vol_of_vol) ** 2 + self._vol_of_vol**2 * (ixi + xi**2))
        ...
        C = (
            ...
            * self._long_run_variance
            / self._vol_of_vol**2
            * ((self._mean_reversion_speed - ixi * self._correlation * self._vol_of_vol - d) * tau - 2.0 * np.log((1 - g * ee) / (1 - g)))
        )
        D = (self._mean_reversion_speed - ixi * self._correlation * self._vol_of_vol - d) / self._vol_of_vol**2 * ((1 - ee) / (1 - g * ee))
    ...
    def call_price(self, s0: float, v0: float, K: Union[np.ndarray, float], ttm: Union[np.ndarray, float]) -> Union[np.ndarray, float]:
        ...
        def integ_func(xi, s0, v0, K, tau, num):
            ...
            if num == 1:
                return (
                    self._characteristic_func(xi - 1j, s0, v0, tau) / (ixi * self._characteristic_func(-1j, s0, v0, tau)) * np.exp(-ixi * np.log(K))
                ).real
            else:
        ...
        if ttm < 1e-3:
            ...
            res = 0.5 * (s0 - K) + 1 / scipy.constants.pi * scipy.integrate.quad_vec(h, 0, 500.0)[0]  # vorher 500
        return res
