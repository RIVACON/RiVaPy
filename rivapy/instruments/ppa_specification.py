...
class SimpleSchedule(interfaces.FactoryObject):
    ...
    def get_schedule(self, refdate: dt.datetime = None) -> np.ndarray:
        ...
        """
        d_ = pd.date_range(
            self.start, self.end, freq=self.freq, tz=self.tz, inclusive="left"
        ).to_pydatetime()
...
class PPASpecification(interfaces.FactoryObject):
    ...
    def _create_sample(n_samples: int, seed: int = None, ref_date=None):
        ...
        for schedule in schedules:
            ...
            result.append(
                PPASpecification(
                    udl="Power", amount=amount, schedule=schedule, fixed_price=fixed_price
                )
            )
        return result
    ...
    def compute_flows(
        self, refdate: dt.datetime, pfc, result: pd.DataFrame = None, result_col=None
    ) -> pd.DataFrame:
        df = pfc.get_df()
        if result is None:
            self._schedule_df["flow"] = self._schedule_df["amount"] * (
                df.loc[self._schedule_df.index]["values"] - self.fixed_price
            )
            return self._schedule_df
        result[result_col] = self._schedule_df["amount"] * (
            df.loc[self._schedule_df.index]["values"] - self.fixed_price
        )
