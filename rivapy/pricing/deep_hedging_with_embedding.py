...
class DeepHedgeModelwEmbedding(tf.keras.Model):
    ...
    def _compute_pnl(self, x_in, training):
        ...
        for i in range(self.timegrid.shape[0] - 2):
            ...
            for j in range(
                len(self.hedge_instruments)
            ):  # we implicitely make the assumption that the additional states come after the hedge instruments in x_in
    ...
    def compute_delta(self, paths: Dict[str, np.ndarray], t: Union[int, float] )->np.ndarray:
        if t is None:
            if False:
                result = np.zeros(
                    (
                ...
                result = self._compute_delta(inputs_, training=False)
                result = np.stack(result, axis=0)
                return result.reshape((self.timegrid.shape[0]-1,-1))
                return result#tf.make_ndarray(result)
        inputs =[] 
        if isinstance(t, int):
            inputs_ = self._create_inputs(paths, check_timegrid=True)
            t_ = (
                [self.timegrid[-1] - self.timegrid[t]]
                * tf.ones((tf.shape(inputs_[0])[0], 1))
                / self.timegrid[-1]
            )
            inputs = self._create_timeslice_input(inputs_, t_, t)
    ...
    def compute_delta_old(
        self,
        paths: Dict[str, np.ndarray],
        t: Union[int, float] = None,
        emb: Union[int, float] = None,
        emb_port: Union[int, float] = None,
    ):
        if t is None:
            result = np.zeros(
                (
    ...
    def load(folder: str):
        ...
        return DeepHedgeModelwEmbedding(
            depth=None, n_neurons=None, model=base_model, **params
        )
    ...
    def get_embedding(self, emb: str)->np.ndarray:
        emb_layer = self.model.get_layer(emb)
        return emb_layer.get_weights()
