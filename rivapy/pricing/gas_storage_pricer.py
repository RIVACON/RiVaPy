...
class PricingParameter:
    def __init__(
        ...
        n_actions: int,
        n_vol_levels: int,
        regression: object = _PolynomialRegressionFunction,
    ):
...
def pricing_lsmc(
    storage: GasStorageSpecification,
    pricing_parameters: PricingParameter,
    prices: np.ndarray,
    nb_sims: int,
    penalty_func: Callable = _penalty_func,
    ...
    acc_cashflows = np.empty(
        (len(storage.timegrid), len(v), nb_sims)
    )  # acc_cashflows[t, i,j]: expected accumulated cashflow (of optimal policy) for j-th path at t arriving at t at vollevel i
    ...
    total_vol_levels = np.empty(
        (len(storage.timegrid), len(v), nb_sims), dtype=int
    )  # total_vol_levels[t,i,j] gives the index of the next optimal volume level in t+1 starting from the i-th volume for the j-th path
    ...
    for t in range(len(storage.timegrid) - 2, -1, -1):
        for vol_t in range(len(v)):
            ...
            for vol_tplus1 in range(len(v)):  # for all volumes by themselves
                ...
                if t > 0:
                    cont_val = acc_cashflows[
                        t + 1, vol_tplus1, :
                    ]  # assumed cont. value for t+1, no disc. factor
    ...
    ind_level[0, :] = (
        0  # on the grid, index of startLevel = 0 TODO: Use index of volume level that is closest to the start level
    )
...
if __name__ == "__main__":
    ...
    def create_contract_dates(
        startdate: dt.datetime, enddate: dt.datetime, datestep: dt.timedelta
    ) -> list:
        dates = [startdate]
    ...
    params = PricingParameter(
        n_time_steps=0, n_actions=0, n_vol_levels=n_vol_levels
    )  # , regression = _PolynomialRegressionFunction)
    store = GasStorageSpecification(
        contractdates,
        storage_capacity,
        max_withdrawal,
        max_injection,
