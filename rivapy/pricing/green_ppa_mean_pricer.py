...
def price(
    ...
    def round_to_hour(t):
        ...
        return t.replace(second=0, microsecond=0, minute=0, hour=t.hour) + dt.timedelta(
            hours=t.minute // 30
        )
    ...
    dg = DateTimeGrid(
        start=round_to_hour(val_date), end=schedule[-1], freq="1H"
    )  # TODO: What to do with the frequency?
    logger.debug("Start model simulation with %d paths.", n_sims)
    results = model.simulate(
        dg,
        n_sims=n_sims,
        start_value_wind=0.5,
        start_value_load=0.5,
