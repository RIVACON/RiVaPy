...
def price(
    val_date: dt.datetime, spec: GreenPPASpecification, model: BaseModel, pr: GreenPPAPricingRequest, n_sims: int = 1000, seed: int = 42
) -> float:
    ...
    dg = DateTimeGrid(start=round_to_hour(val_date), end=schedule[-1], freq="1h")  # TODO: What to do with the frequency?
    logger.debug("Start model simulation with %d paths.", n_sims)
    results = model.simulate(dg, n_sims=n_sims, start_value_wind=0.5, start_value_load=0.5, start_value_solar=0.5, seed=seed)
    ...
    result = {}
    quantity = results[spec.technology][
        index, :
    ]  # overall total quantity produced. This must be scaled by capacity of the plant in relation to the total capacity
...
if __name__ == "__main__":
    ...
    green_ppa = GreenPPASpecification(simple_schedule, fixed_price=10.0, max_capacity=10, technology="wind", udl="power", location="")
