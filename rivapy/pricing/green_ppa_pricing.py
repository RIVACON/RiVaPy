...
class GreenPPADeepHedgingPricer:
    class PricingResults:
        def __init__(
            self, hedge_model: DeepHedgeModel, paths: np.ndarray, sim_results, payoff
        ):
    ...
    def compute_payoff(
        ...
        for (
            ...
            hedge_ins.items()
        ):  # TODO: We assume that each hedge instruments corresponds to the spot price at the last time step. Make this more explicit!
            expiry = k.split("_")[-1]
            forecast_key = green_ppa.location + "_" + expiry
            payoff += (
                (v[-1, :] - green_ppa.fixed_price)
                * (additional_states[forecast_key][-1, :])
                * green_ppa.max_capacity
            )
        return payoff
    ...
    def generate_paths(
        ...
        if timegrid is None:
            timegrid, expiries, _ = GreenPPADeepHedgingPricer._compute_points(
                val_date, green_ppa, [0]
            )
        rnd = np.random.normal(
            size=power_wind_model.rnd_shape(n_sims, timegrid.timegrid.shape[0])
        )
        return power_wind_model.simulate(
            timegrid.timegrid,
            rnd,
            expiries=expiries,
    ...
    def price(
        ...
        if green_ppa.udl not in power_wind_model.udls():
            raise Exception(
                "Underlying "
                + green_ppa.udl
                + " not in underlyings of the model "
                + str(power_wind_model.udls())
            )
        tf.random.set_seed(seed)
        np.random.seed(seed + 123)
        timegrid, expiries, forecast_points = GreenPPADeepHedgingPricer._compute_points(
            val_date, green_ppa, forecast_hours
        )
        if len(expiries) == 0:
            return None
        rnd = np.random.normal(
        ...
        simulation_results = power_wind_model.simulate(
            timegrid.timegrid,
            rnd,
            expiries=expiries,
            initial_forecasts=initial_forecasts,
        ...
        if additional_states is not None:
            for a in additional_states:
                for i in range(len(expiries)):
                    ...
                    additional_states_[key] = simulation_results.get(
                        key, forecast_points
                    )

        hedge_model = DeepHedgeModel(
            list(hedge_ins.keys()),
            list(additional_states_.keys()),
            timegrid.timegrid,
        ...
        payoff = GreenPPADeepHedgingPricer.compute_payoff(
            n_sims, hedge_ins, additional_states_, green_ppa
        )
        ...
        return GreenPPADeepHedgingPricer.PricingResults(
            hedge_model, paths=paths, sim_results=simulation_results, payoff=payoff
        )
