...
class SimpleCashflowPricer:
    @staticmethod
    def pv_cashflows(
        val_date: datetime, specification: HasExpectedCashflows, discount_curve: BaseDatedCurve
    ) -> float:
        ...
        for c in cashflows:
            if c[0] >= val_date:
                ...
                pv_cashflows += df * c[1]
        logger.info(
            "Finished computing pv cashflows for bond "
            + specification.obj_id
            + ", pv_cashflows: "
            + str(pv_cashflows)
        )
        return pv_cashflows
    ...
    @staticmethod
    def compute_yield(
        target_dirty_price: float, val_date: datetime, specification: HasExpectedCashflows
    ) -> float:
        logger.info(
            "Start computing bond yield for bond "
            + specification.obj_id
            + ", dirty price: "
            + str(target_dirty_price)
        )
