...
from rivapy.tools.datetools import Period, Schedule, calc_end_day, calc_start_day, term_to_period, datetime_to_date
from rivapy.tools.enums import SecuritizationLevel
from rivapy.tools._converter import converter as _converter, create_ptime as _create_ptime
...
)  # do not use method YieldCurveBootstrapper_compute
...
class Par_IR_Data:
    def __init__(self, data_file: DataFrame, ref_date: _Union[date, datetime] = None):
        ...
        self.underlying_pay_freq = data_file[intersect(headings, ("Underlying Payment Frequency", "UnderlyingPaymentFrequency"))]
        ...
        self.roll_conv_fix = data_file[intersect(headings, ("Roll Convention Fixed", "RollConventionFixed", "Business Day Convention Fixed"))]
        self.roll_conv_fix = data_file[intersect(headings, ("Roll Convention Float", "RollConventionFloat", "Business Day Convention Float"))]
        self.roll_conv_fix = data_file[intersect(headings, ("Roll Convention Basis", "RollConventionBasis", "Business Day Convention Basis"))]
    ...
    def specify_irs(self):
        ...
        ir_swap = InterestRateSwapSpecification(
            self.label, "dummy_issuer", SecuritizationLevel.COLLATERALIZED, self.currency, expiry_date, fixed_leg, float_leg
        )
        return ir_swap
    ...
    def get_tbs_spec(self):
        ...
        basis_swap = InterestRateBasisSwapSpecification(
            self.label, "dummy_issuer", SecuritizationLevel.COLLATERALIZED, self.currency, expiry_date, float_leg_basis, float_leg, fixed_leg
        )
        return basis_swap
    ...
    def get_fra_spec(self):
        ...
        # specification of the deposit
        fra = InterestRateFutureSpecification(
            self.label,
            "dummy_issuer",
            SecuritizationLevel.NONE,
            self.currency,
    ...
    def get_float_leg(self, pay_freq, reset_freq, roll_conv, spot_lag="0D"):
        ...
        float_leg = IrFloatLegSpecification(
            flt_notionals, flt_reset_dates, flt_start_dates, flt_end_dates, flt_pay_dates, self.currency, "dummy_undrl", self.float_day_count, 0.0
        )
        return float_leg
    ...
    def get_fix_leg(self, pay_freq, roll_conv, spot_lag="0D"):
        ...
        fixed_leg = IrFixedLegSpecification(
            self.par_rate, fix_notionals, fix_start_dates, fix_end_dates, fix_pay_dates, self.currency, self.fix_day_count
        )
        return fixed_leg
...
def bootstrap_curve(data_file: DataFrame, output_spec: dict) -> DataFrame:
    ...
    if len(ref_date) != 1:
        raise Exception("The output specification must include (exactly) one key " + str(ref_date_keys) + " defining the reference date!")
    del ref_date_keys
    ...
    if len(discount_curve) == 1:
        ...
        raise Exception("The output specification must not include more than one key " + str(discount_curve_keys) + " defining the discount curve!")
    ...
    if len(basis_curve) == 1:
        ...
        raise Exception("The output specification must not include more than one key " + str(basis_curve_keys) + " defining the basis curve!")
    del basis_curve_keys

    curve = YieldCurveBootstrapper.compute(
        ref_date, output_spec["curveName"], output_spec["dayCount"], instruments, quotes, discount_curve, basis_curve
    )
    return curve
...
class YieldCurveBootstrapper:
    ...
    def compute(
        ...
        assert len(quotes) == len(instruments), "Mismatch in instruments and quotes."

        yc_dates = [ref_date]
        dfs = [1.0]
        dc = DayCounter(dc_type)
        ...
        # Curve constructor logic
        base_curve = next((c for c in curves if c.get_object_id() == base_curve_name), None) if base_curve_name else None

        def make_curve(df_vals: List[float], dates: List[datetime], interp: str, extrap: str) -> "DiscountCurve":
            raw_curve = DiscountCurve(obj_id, ref_date, dates, df_vals, dc_type, interp, extrap)
            if base_curve:
                return DiscountCurveShifted(obj_id, raw_curve, base_curve)
            return raw_curve

        # Initial pass with LINEARLOG to get initial guesses
        for end_date in sorted(instruments_by_date):
            quote, inst = instruments_by_date[end_date]
            yc_dates.append(end_date)
            dfs.append(dfs[-1])

            def error_fn(df_val: float):
                df_tmp = dfs.copy()
                df_tmp[-1] = df_val
                yc = make_curve(df_tmp, yc_dates, "LINEARLOG", "LINEARLOG")
                return inst.get_quote(ref_date, curves + [yc]) - quote

            try:
                solution = brentq(error_fn, 0.00001, 5.0, xtol=1e-5)
                dfs[-1] = solution
        ...
        while iteration < max_iterations and (max_diff > tolerance or iteration == 0):
            ...
            for i, end_date in enumerate(sorted(instruments_by_date), start=1):
                ...
                def error_fn(df_val: float):
                    df_tmp = dfs.copy()
                    df_tmp[i] = df_val
                    yc = make_curve(df_tmp, yc_dates, interpolation_type, extrapolation_type)
                    return inst.get_quote(ref_date, curves + [yc]) - quote

                try:
                    tol_brent = dfs[i] * tolerance * dc.year_fraction(ref_date, end_date) * 0.1
            ...
            for i, end_date in enumerate(sorted(instruments_by_date), start=1):
                quote, inst = instruments_by_date[end_date]
                yc = make_curve(dfs, yc_dates, interpolation_type, extrapolation_type)
                q_model = inst.get_quote(ref_date, curves + [yc])
                epsilon = 1e-6
                dfs_perturbed = dfs.copy()
                dfs_perturbed[i] += epsilon
                yc_perturbed = make_curve(dfs_perturbed, yc_dates, interpolation_type, extrapolation_type)
                q_model_eps = inst.get_quote(ref_date, curves + [yc_perturbed])

                dq = (q_model_eps - q_model) / epsilon
                dr = abs((quote - q_model) / (dq * dc.year_fraction(ref_date, end_date) * dfs[i]))
        ...
        return make_curve(dfs, yc_dates, interpolation_type, extrapolation_type)
