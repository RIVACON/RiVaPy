...
class PFCShifter(interfaces.FactoryObject):
    ...
    def detect_redundant_contracts(self, transition_matrix: pd.DataFrame) -> pd.DataFrame:
        ...
        if len(potential_redundant_contracts) != 0:
            for contract_idx in potential_redundant_contracts:
                _temp_matrix = np.concatenate(
                    [base_matrix, np_transition_matrix[contract_idx, :].reshape(1, -1)], axis=0
                )
    ...
    def shift(self, transition_matrix: pd.DataFrame) -> pd.DataFrame:
        contract_start_and_end_dates = np.array(self._get_contract_start_end_dates())
        contract_schedules = np.unique(
            list(itertools.chain(*[contract.get_schedule() for contract in self.contracts.values()]))
        )
        ...
        delivery_ticks_per_period = np.concatenate(
            [np.array([delivery_ticks[0]]), (delivery_ticks[1:] - delivery_ticks[:-1])]
        )
        ...
        for i, date_tpl in enumerate(date_tpls):
            ...
            pfc.iloc[row_filter, 0] = (
                pfc.iloc[row_filter, 0]
                / np.sum(pfc.iloc[row_filter, 0])
                * len(pfc.iloc[row_filter, 0])
                * fwd_price_noc[i, 0]
            )
        return pfc
