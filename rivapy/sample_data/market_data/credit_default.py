...
class CreditDefaultData:
    @staticmethod
    def sample(n_data: int, seed: int = None, constant=-1.0, cov: np.ndarray = None) -> pd.DataFrame:
        ...
        The single features (exception is the region that is drawn independently of the other features) are related via a Gaussian copula. The following figure shows the distributions and pairplots for a
        ...
        Args:
            ...
            cov (np.ndarray, optional): Covariance matrix used in the Gaussian copula. Defaults to None (then a flat covariance of 0.95 is used).
...
class CreditDefaultData2:
    ...
    @staticmethod
    def sample(n_years: int, n_data_per_year: int, seed: int = None, cov: np.ndarray = None, include_economic_factor: bool = True) -> pd.DataFrame:
        """Sample credit default data.
        ...
        if include_economic_factor:
            x = pd.DataFrame(np.empty((n_years * n_data_per_year, 5)), columns=["age", "income", "savings", "credit_income_ratio", "economic_factor"])
        else:
            x = pd.DataFrame(np.empty((n_years * n_data_per_year, 4)), columns=["age", "income", "savings", "credit_income_ratio"])
...
class CreditDefaultDataCategoricalFeature:
    ...
    def sample(n_years: int, n_data_per_year: int, seed: int = None, cov: np.ndarray = None, include_economic_factor: bool = True) -> pd.DataFrame:
        ...
        The single features (except the zip code) are related via a Gaussian copula. The following figure shows the distributions and pairplots for a
        generated sample of features.
