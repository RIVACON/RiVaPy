...
class SpreadCurveCollection(FactoryObject):
    ...
    def __init__(
        ...
        by a linear combination of different spreadcurves that belong to certain bond features
            ...
            - For each rating :math:`r^{\\star}` (issuer rating) a spreadcurve :math:`S_{r^\\star}`
        ...
        self.rating_curve = [
            SpreadCurveCollection._create_curve_or_float(rating_curve[0]),
            SpreadCurveCollection._create_curve_or_float(rating_curve[1]),
        ]
    ...
    def get_curve(self, issuer: Issuer, bond: PlainVanillaCouponBondSpecification):
        ...
        curve = 0.5 * rating_curve + 0.5 * (
            0.3 * country_spread + 0.3 * securitization_spread + 0.2 * esg_spread + 0.1 * sector_spread + 0.1 * currency_spread
        )
        return curve
...
class SpreadCurveSampler:
    def __init__(self, sector_weights=None, country_weights=None):
        ...
        * securitization level (only SENIOR_SECURED, SENIOR_UNSECURED and SUBORDINATED are currently handled)
        ...
        of this parametrization. Each curve is constructed so that for all of the features above fixed, the curve is consistent w.r.t. the issuer rating in
    ...
    def sample(self, ref_date: dt.datetime) -> dict:
        ...
        curve_best_rating = DiscountCurveParametrized("", ref_date, NelsonSiegel._create_sample(n_samples=1, seed=None, **min_params)[0])
        curve_worst_rating = curve_best_rating + DiscountCurveParametrized(
            "", ref_date, NelsonSiegel._create_sample(n_samples=1, seed=None, **max_params)[0]
        )
        ...
        return {
            "rating_curves": self.rating_curve,
            "currency_spread": self.currency_spread,
            "esg_rating_spread": self.esg_rating_spread,
            "rating_weights": self.rating_weights,
            "sector_spreads": self.sector_spreads,
            "country_curves": self.country_curves,
    ...
    def sample_new(self, ref_date: dt.datetime) -> SpreadCurveCollection:
        ...
        self._sample_country_curves(ref_date=ref_date)
        self._sample_sec_level_spreads()
        self.spread_curve_collection = SpreadCurveCollection(
            ref_date,
            self.rating_curve,
    ...
    def _sample_country_curves(self, ref_date):
        ...
        if self.country_weights is None:
            for c in Country:
                ...
                self.country_curves[c.value] = (
                    lower_curve,
                    lower_curve + DiscountCurveParametrized("", ref_date, ConstantRate(np.random.uniform(0.05, 0.15))),
                )
        else:
            ...
            for i, c in enumerate(Country):
                ...
                self.country_curves[c.value] = (lower_curve, lower_curve + DiscountCurveParametrized("", ref_date, ConstantRate(rating_offset[i])))
    ...
    def get_curve(self, issuer: Issuer, bond: PlainVanillaCouponBondSpecification):
        ...
        sector_spread = w1 * self.sector_spreads[issuer.sector][0] + w2 * self.sector_spreads[issuer.sector][1]
        currency_spread = w1 * self.currency_spread[bond.currency][0] + w2 * self.currency_spread[bond.currency][1]
        securitization_spread = (
            w1 * self.securitization_spreads[bond.securitization_level][0] + w2 * self.securitization_spreads[bond.securitization_level][1]
        )
        curve = 0.5 * rating_curve + 0.5 * (
            0.3 * country_spread + 0.3 * securitization_spread + 0.2 * esg_spread + 0.1 * sector_spread + 0.1 * currency_spread
        )
