...
class LocalVolModelTest(unittest.TestCase):
    ...
    def test_local_vol_mc_with_ssvi(self):
        ...
        if not _pyvacon_available:
            ...
            return
        ssvi = mktdata.VolatilityParametrizationSSVI(
            expiries=[1.0 / 365, 30 / 365, 0.5, 1.0], fwd_atm_vols=[0.25, 0.3, 0.28, 0.25], rho=-0.9, eta=0.5, gamma=0.5
        )
    ...
    def test_local_vol_with_flat_input_vol(self):
        ...
        call_prices = np.array(
            [[analytics.compute_european_price_Buehler(strike=k, maturity=t, volatility=input_vol) for k in x_strikes] for t in time_grid]
        )
    ...
    def test_compare_local_var_implied_and_call(self):
        ...
        call_prices = np.array(
            [
                [analytics.compute_european_price_Buehler(strike=k, maturity=t, volatility=input_vol[i, j]) for j, k in enumerate(x_strikes)]
                for i, t in enumerate(time_grid)
            ]
        )
...
class HestonModelTest(unittest.TestCase):
    ...
    def test_callprice_formula(self):
        ...
        heston = models.HestonModel(long_run_variance=0.3**2, mean_reversion_speed=0.5, vol_of_vol=0.2, initial_variance=0.1**2, correlation=-0.9)
...
class HestonLocalVolModelTest(unittest.TestCase):
    ...
    def calc_imlied_vol_grid(expiries, strikes, call_prices):
        ...
        for i in range(expiries.shape[0]):
            for j in range(strikes.shape[0]):
                try:
                    vols[i, j] = analytics.compute_implied_vol_Buehler(strikes[j], maturity=expiries[i], price=call_prices[i, j], min_vol=0.01)
                except:
                    pass
    ...
    def test_simple(self):
        ...
        heston = models.HestonModel(long_run_variance=0.3**2, mean_reversion_speed=0.5, vol_of_vol=0.2, initial_variance=0.3**2, correlation=-0.95)
        ...
        for t in [80, 120, 180, 239]:
            for k in [80, 100, 120, 140, 160]:
                vol = analytics.compute_implied_vol_Buehler(x_strikes[k], maturity=time_grid[t], price=call_prices[t, k], min_vol=0.01)
                vol_sim = analytics.compute_implied_vol_Buehler(x_strikes[k], maturity=time_grid[t], price=call_prices_sim[t, k], min_vol=0.01)
                self.assertTrue(
                    np.abs(vol - vol_sim) < 0.02,
                    "Vol from calibrated model ("
                    + str(vol_sim)
                    + ") is not close enough to reference vol("
                    + str(vol)
                    + ") for strike/expiry: "
                    + str(x_strikes[k])
                    + "/"
    ...
    def test_simple_2(self):
        ...
        heston = models.HestonModel(long_run_variance=0.3**2, mean_reversion_speed=0.5, vol_of_vol=0.2, initial_variance=0.3**2, correlation=-0.95)
        heston_2 = models.HestonModel(long_run_variance=0.2**2, mean_reversion_speed=0.5, vol_of_vol=0.2, initial_variance=0.3**2, correlation=-0.95)
        x_strikes = np.linspace(0.5, 1.5, num=240)
        time_grid = np.linspace(0.0, 1.0, num=240)
        call_prices = heston.call_price(1.0, heston._initial_variance, x_strikes, time_grid)
