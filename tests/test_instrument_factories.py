...
class SerializationTestsMeta(type):
    
    def __new__(cls, name, bases, attrs):
        for k,v in _factory().items():
            attrs['test_%s' % k] = cls.gen(v)
        return super(SerializationTestsMeta, cls).__new__(cls, name, bases, attrs)
    ...
    def gen(cls, target_class):
        ...
        def fn(self):
            try:
                ins = target_class._create_sample(4, seed=42)
                for i in range(len(ins)):
                    b = target_class.from_json(ins[i].to_json())
                    self.assertEqual(b.hash(), ins[i].hash())
            except AttributeError as e:
                self.assertEqual(0,1, msg = '_create_sample not implemented for class ' + target_class.__name__)            
        return fn
...
class InstrumentSerializationTests(unittest.TestCase, metaclass = SerializationTestsMeta):
    pass
