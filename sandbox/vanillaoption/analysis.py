...
class Repo:
    ...
    def run(self, val_date, spec, model, rerun=False, **kwargs):
        ...
        params['model'] = {model[k].modelname: model[k].to_dict() for k in range(len(model))}#model.to_dict()
        ...
        params['model_hash'] = {model[k].modelname: model[k].hash() for k in range(len(model))}
