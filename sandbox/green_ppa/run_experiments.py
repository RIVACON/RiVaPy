...
wind_onshore = WindPowerForecastModel(
    region="Onshore", speed_of_mean_reversion=0.1, volatility=3.0
)
wind_offshore = WindPowerForecastModel(
    region="Offshore", speed_of_mean_reversion=0.1, volatility=3.0
)
regions = [
    MultiRegionWindForecastModel.Region(
        wind_onshore, capacity=1000.0, rnd_weights=[0.8, 0.2]
    ),
    MultiRegionWindForecastModel.Region(
        wind_offshore, capacity=100.0, rnd_weights=[0.2, 0.8]
    ),
]
...
for max_capacity in [
    1.0
]:  # [0.0, 0.125, 0.25, 0.5, 0.75, 1.0, 1.25, 1.5, 1.75, 2.0, 5.0, 10.0]:
    ...
    for x_volatility in [0.8]:  # [0.8]
        for vol_wind_onshore in [3.0]:  # [1.0,2.0,3.0,4.0]: #
            for wind_onshore_ in [0.2, 0.5, 0.8]:
                ...
                wind = MultiRegionWindForecastModel("Wind_Germany", regions)
                model = LinearDemandForwardModel(
