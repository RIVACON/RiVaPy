...
class Repo:
    ...
    def run(self, val_date, ppa_spec, model, rerun=False, **kwargs):
        ...
        pricing_result = GreenPPADeepHedgingPricer.price(
            val_date, ppa_spec, model, **kwargs
        )
    ...
    def simulate_model(self, hashkey: str, n_sims: int, seed: int = 42) -> np.ndarray:
        ...
        rnd = np.random.normal(
            size=model.rnd_shape(n_sims=n_sims, n_timesteps=timegrid.shape[0])
        )
    ...
    def plot_hiplot(
        self,
        error="mean_rmse_scaled_recalib",
        error_train="mean_rmse_train_scaled",
        ...
        for k, v in self.results.items():
            ...
            tmp["vol_onshore"] = v["model"]["wind_power_forecast"][
                "region_forecast_models"
            ][0]["model"]["volatility"]
            ...
            region_forecast_models = v["model"]["wind_power_forecast"][
                "region_forecast_models"
            ]
...
def plot_paths(paths: ForwardSimulationResult, forecast_points, result_dir: str = None):
